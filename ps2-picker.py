...
def _detect_distro():
    ...
    pretty = info.get('PRETTY_NAME', 'Linux')
...
def check_dependencies(force=False):
    """Check all dependencies. Returns list of (name, installed, install_cmd) tuples.
    If force=True, returns results even when everything is installed."""
    distro = _detect_distro()

    # Define install commands per dependency per distro
    install_cmds = {
        'windows': {
        ...
        'steamos': {
            'pygame':    'pip install --user pygame  (after disabling read-only: steamos-readonly disable)',
            '7z':        'sudo pacman -S p7zip  (after steamos-readonly disable)',
            'retroarch': 'flatpak install org.libretro.RetroArch',
        },
        'batocera': {
            'pygame':    'Pre-installed on Batocera (check your image version)',
            '7z':        'Pre-installed on Batocera (check your image version)',
    ...
    fallback = {
        ...
        'retroarch': 'Install retroarch via your package manager or https://retroarch.com',
    }

    cmds = install_cmds.get(distro, fallback)

    deps = [
        ('pygame',    'Python game library (required)',  _check_python_module('pygame'), cmds.get('pygame', fallback['pygame'])),
        ('7z',        '7-Zip archive tool (required for .7z)', _check_command('7z'),       cmds.get('7z', fallback['7z'])),
        ('retroarch', 'RetroArch emulator (required)',   _check_command('retroarch'),    cmds.get('retroarch', fallback['retroarch'])),
    ]

    missing = [d for d in deps if not d[2]]

    if force or missing:
        return deps, distro, missing
    return None, distro, []


def _print_dep_report(deps, distro, missing):
    """Terminal fallback when pygame is not available."""
    pretty = _get_distro_pretty_name()
    print("")
    print("═" * 60)
    print("  PS2 Picker — Dependency Check")
    print(f"  Detected OS: {pretty} ({distro})")
    print("═" * 60)
    for name, desc, installed, cmd in deps:
...
HAS_PYGAME = _check_python_module('pygame')

# Handle --check-deps before pygame init
if '--check-deps' in sys.argv:
    deps, distro, missing = check_dependencies(force=True)
    if not HAS_PYGAME:
        _print_dep_report(deps, distro, missing)
        sys.exit(1 if missing else 0)
    # If pygame is available, we'll show the GUI version after init
...
def show_dependency_screen(deps, distro, missing, force=False):
    """GUI screen showing dependency status with install commands."""
    pretty = _get_distro_pretty_name()
    ...
    while True:
        ...
        # Title
        title = F['xl'].render("Dependency Check", True, HDR)
        screen.blit(title, title.get_rect(center=(W // 2, scaled(20))))

        # Detected OS
        os_text = F['sm'].render(f"Detected: {pretty} ({distro})", True, ACCENT)
        screen.blit(os_text, os_text.get_rect(center=(W // 2, scaled(42))))

        pygame.draw.line(screen, ACCENT, (scaled(10), scaled(55)), (W - scaled(10), scaled(55)), 1)

        # Build content lines
        lines = []
        for name, desc, installed, cmd in deps:
            if installed:
                lines.append(("✓  " + name + "  —  " + desc, SUCCESS, False))
            else:
                lines.append(("✗  " + name + "  —  " + desc, DANGER, False))
                lines.append(("   Install:  " + cmd, HINT, True))
        if all_ok:
            lines.append(("", None, False))
            lines.append(("✓  All dependencies satisfied!", SUCCESS, False))
        else:
            lines.append(("", None, False))
            lines.append(("Install missing dependencies and restart.", DANGER, False))

        # Render scrollable lines
        y_start = scaled(65)
        line_h = scaled(22)
        max_vis = (H - scaled(90)) // line_h
        max_scroll = max(0, len(lines) - max_vis)
        scroll = min(scroll, max_scroll)

        for i in range(scroll, min(scroll + max_vis, len(lines))):
            text, color, is_cmd = lines[i]
            if not text:
                continue
            font = F['sm'] if is_cmd else F['md']
            rendered = font.render(text, True, color)
            y_pos = y_start + (i - scroll) * line_h
            screen.blit(rendered, (scaled(14), y_pos))

        # Bottom hint
        if all_ok or force:
            draw_hint_bar("[A] Continue   [B] Exit")
        else:
            draw_hint_bar("Press any button to exit")
